...
struct component {
  ...
  using initial_tags = tmpl::flatten<tmpl::list<
      ...
      // to add bool enable_extension_directions
...
void add_rdmp_data_to_datavector(
    DataVector& data_to_fill, const size_t original_data_size,
    const evolution::dg::subcell::RdmpTciData& rdmp_tci_data) {
...
DataVector compute_expected_ghost_data(
  ...
  const auto& fd_to_neighbor_fd_interpolants = ActionTesting::get_databox_tag<
      comp, evolution::dg::subcell::Tags::InterpolatorsFromFdToNeighborFd<Dim>>(
      runner, element_id);
  const auto& extension_directions = ActionTesting::get_databox_tag<
  ...
  if (enable_extension and extension_directions.contains(direction)) {
    ...
    relevant_ghost_data.set_data_ref(
        const_cast<double*>(data_from_extender.data()),
        data_from_extender.size() - rdmp_size);
    const size_t ghost_data_size = relevant_ghost_data.size();

    DataVector voluem_data_to_slice = [&evolved_vars, use_cell_centered_flux,
                                       &runner, &element_id]() {
      if (use_cell_centered_flux) {
        const auto& cell_centered_flux =
            ActionTesting::get_databox_tag<comp, CellCenteredFluxTag>(
                runner, element_id);
        DataVector buffer{evolved_vars.size() +
                          cell_centered_flux.value().size()};
        std::copy(evolved_vars.data(),
    ...
    for (size_t i = 0; i < evolved_vars.size(); ++i) {
      voluem_data_to_slice[i] *= 2.0;
    }
    const DataVector combined_data =
        evolution::dg::subcell::combine_volume_ghost_data(
            voluem_data_to_slice, relevant_ghost_data, subcell_mesh.extents(),
    ...
    add_rdmp_data_to_datavector(exepected_ghost_data, ghost_data_size,
                                rdmp_tci_data);
    ...
    add_rdmp_data_to_datavector(expected_ghost_data, sliced_data_size,
                                rdmp_tci_data);
...
template <size_t Dim, bool UseNodegroupDgElements>
void extra_test(const bool enable_extension,
                const bool use_cell_centered_flux) {
  CAPTURE(Dim);
  CAPTURE(enable_extension);
  CAPTURE(use_cell_centered_flux);
  // create Sphere domain, and 3 releveant elements.
...
SPECTRE_TEST_CASE("Unit.Evolution.Subcell.Actions.ReconstructionCommunication",
  ...
  for (const bool use_cell_centered_flux : {false, true}) {
    ...
    extra_test<3, false>(false, use_cell_centered_flux);
    extra_test<3, false>(true, use_cell_centered_flux);
  }
}
}  // namespace
