...
void test_variable_fixer(
  ...
  // comoing magnetic field squared, b^2 = 8.e-4 given values of spatial
  // metric and spatial_velocity specified below.
  magnetic_field.get(0) = DataVector{0.0, 0.0, 0.0, 2.e-4};

  Scalar<DataVector> lorentz_factor{DataVector{5.0 / 3.0, 7.0710678118654752,
                                               1.8898223650461359,
  ...
  // W = 1.41421356237309492343, W_perp = 1.0
  // old_enthalpy/new_enthalpy = 0.50000000010000001
  ...
  if (use_magnetization_limiting) {
    expected_spatial_velocity.get(0)[3] = 0.36602540382670351038;
    get(expected_lorentz_factor)[3] = 1.16877089452976568040;
  }
...
void test_variable_fixer(
  ...
  Scalar<DataVector> lorentz_factor{
      DataVector{get(density).size(), 1.4142135623730949}};
  auto spatial_velocity =
      make_with_value<tnsr::I<DataVector, Dim, Frame::Inertial>>(density, 0.);
  ...
  const Scalar<DataVector> expected_density{
      DataVector{get(density).size(), 8.e-10}};
  const Scalar<DataVector> expected_pressure{
      DataVector{4.e-11, 4.e-10, 4.e-11, 8.e-10}};
  auto expected_specific_internal_energy =
      equation_of_state.specific_internal_energy_from_density_and_pressure(
          expected_density, expected_pressure);
  const Scalar<DataVector> expected_lorentz_factor{
      DataVector{1.1687708944803674, 1.3070017251313011, 1.3878639724008113,
                 1.4142135623730949}};
  auto expected_spatial_velocity =
      make_with_value<tnsr::I<DataVector, Dim, Frame::Inertial>>(density, 0.0);
  expected_spatial_velocity.get(0) = DataVector{
      {0.36602540378443860, 0.45530582753326948, 0.49032339135255509, 0.5}};
