...
template void four_velocity(const gsl::not_null<tnsr::A<DataVector, 3>*>,
                            const tnsr::I<DataVector, 3>&,
                            const tnsr::I<DataVector, 3>&,
                            const Scalar<DataVector>&,
                            const Scalar<DataVector>&);

template void four_velocity(const gsl::not_null<tnsr::A<double, 3>*>,
                            const tnsr::I<double, 3>&,
                            const tnsr::I<double, 3>&, const Scalar<double>&,
                            const Scalar<double>&);
...
void stress_energy_tensor(
  ...
  TempBuffer<tmpl::list<::Tags::TempScalar<0, DataType>,
      ...
      buffer(get_size(rest_mass_density));
  ...
  for (size_t i = 0; i < 4; ++i) {
    result->get(i, i) = get(rho_h_star) * square(four_velocity_v.get(i)) +
                        get(p_star) * inverse_spacetime_metric_v.get(i, i) -
