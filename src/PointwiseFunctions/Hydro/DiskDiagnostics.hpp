...
template <typename DataType>
void mdot(gsl::not_null<Scalar<DataType>*> result,
...
template <typename DataType>
void bdot(gsl::not_null<Scalar<DataType>*> result,
...
template <typename DataType>
struct Mdot : db::SimpleTag {
  using type = Scalar<DataType>;
};

template <typename DataType>
struct Bdot : db::SimpleTag {
  using type = Scalar<DataType>;
};
...
/// Can be retrieved using `hydro::Tags::Mdot`
template <typename DataType, typename OutputCoordsTag>
struct MdotCompute : Mdot<DataType>, db::ComputeTag {
  using base = Mdot<DataType>;
  using return_type = typename base::type;
  using argument_tags =
      tmpl::list<RestMassDensity<DataType>, LorentzFactor<DataType>,
                 SpatialVelocity<DataType, 3>, ::gr::Tags::Lapse<DataType>,
                 ::gr::Tags::Shift<DataType, 3, Frame::Inertial>,
                 ::gr::Tags::SpatialMetric<DataType, 3, Frame::Inertial>,
                 OutputCoordsTag>;
  ...
  static constexpr auto function = static_cast<void (*)(
      ...
      const Scalar<DataType>& lapse, const tnsr::I<DataType, 3>& shift,
      const tnsr::ii<DataType, 3>& spatial_metric,
      const tnsr::I<DataType, 3>& coordinates)>(&hydro::mdot<DataType>);
};

/// Can be retrieved using `hydro::Tags::Bdot`
template <typename DataType, typename OutputCoordsTag>
struct BdotCompute : Bdot<DataType>, db::ComputeTag {
  using base = Bdot<DataType>;
  using return_type = typename base::type;
  using argument_tags =
      tmpl::list<SpatialVelocity<DataType, 3>,
                 ::gr::Tags::SpatialMetric<DataType, 3, Frame::Inertial>,
                 OutputCoordsTag>;

  static constexpr auto function =
      static_cast<void (*)(gsl::not_null<Scalar<DataType>*> result,
                           const tnsr::I<DataType, 3>& spatial_velocity,
                           const tnsr::ii<DataType, 3>& spatial_metric,
                           const tnsr::I<DataType, 3>& coordinates)>(
          &hydro::bdot<DataType>);
