...
class PolarNada : public virtual evolution::initial_data::InitialData,
                  public MarkAsAnalyticData,
                  public AnalyticDataBase {
 public:
  struct NadaParameters {
    using type = Nada;
  ...
  using options = tmpl::list<NadaParameters, TorusParameters>;

  static constexpr Options::String help =
      "Stationary disk in polar coordinates.";

  PolarNada() = default;
  PolarNada(const PolarNada& /*rhs*/) = default;
  PolarNada& operator=(const PolarNada& /*rhs*/) = default;
  PolarNada(PolarNada&& /*rhs*/) = default;
  PolarNada& operator=(PolarNada&& /*rhs*/) = default;
  ~PolarNada() override = default;

  PolarNada(Nada nada, grmhd::AnalyticData::SphericalTorus torus_map);

  auto get_clone() const
      -> std::unique_ptr<evolution::initial_data::InitialData> override;

  /// \cond
  explicit PolarNada(CkMigrateMessage* msg);
  using PUP::able::register_constructor;
  WRAPPED_PUPable_decl_template(PolarNada);
  /// \endcond
  ...
  tuples::TaggedTuple<Tags...> variables(const tnsr::I<DataType, 3>& x,
    ...
    auto observation_data =
        nada_.variables(observation_coordinates, required_tags{});
  ...
  using equation_of_state_type = Nada::equation_of_state_type;
  const equation_of_state_type& equation_of_state() const {
    return nada_.equation_of_state();
  }
 ...
 private:
  friend bool operator==(const PolarNada& lhs, const PolarNada& rhs);
