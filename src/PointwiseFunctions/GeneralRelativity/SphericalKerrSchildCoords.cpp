...
SphericalKerrSchildCoords::SphericalKerrSchildCoords(
...
void SphericalKerrSchildCoords::pup(PUP::er& p) { p | spin_a_; }

template <typename DataType>
tnsr::Ij<DataType, 3, Frame::NoFrame>
SphericalKerrSchildCoords::jacobian_matrix(const DataType& x, const DataType& y,
                                           const DataType& z) const {
  auto result = make_with_value<tnsr::Ij<DataType, 3, Frame::NoFrame>>(x, 0.0);
  DataType r_squared = square(x)+square(y)+square(z);
  DataType r = sqrt(r_squared);
  DataType prefactor = sqrt(1 - (square(z) / r_squared));

  get<0, 0>(result) = x / r;
  get<0, 1>(result) = x * z / (r * prefactor);
  get<0, 2>(result) = -y;
  get<1, 0>(result) = y / r;
  get<1, 1>(result) = y * z / (r * prefactor);
  get<1, 2>(result) = x;
  get<2, 0>(result) = z / r;
  get<2, 1>(result) = -r * prefactor;
...
SphericalKerrSchildCoords::cartesian_from_spherical_ks(
