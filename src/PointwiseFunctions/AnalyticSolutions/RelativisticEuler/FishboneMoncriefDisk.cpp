...
FishboneMoncriefDisk::variables(
  ...
  variables_impl(vars, [&spatial_velocity, &vars, &x, this](
    ...
    // and then transform this vector back to Spherical-Kerr-Schild coords.
    // "sks_to_ks_factor" accounts for the x,y now in Spherical-Kerr-Schild.
    ...
    const auto inv_jacobians =
        get<inv_jacobian>(background_spacetime_.variables(
    ...
    for (size_t i = 0; i < 3; ++i) {
      get_element(spatial_velocity.get(i), s) =
          (gsl::at(transport_velocity_sks, i) +
           get_element(
