 ...
 *
 * \note
 * This documentation outlines the algebra for computing the spatial velocity
 * \f$ v^i \f$:
 *
 ...
 *     &= \left(\rho h + \frac{B^2}{W^2} + (B \cdot v)^2\right) W^2 v_i
 *        - \alpha \left(\frac{W}{\alpha}(B \cdot v)\right)
 *        \left(\frac{B_i}{W} + (B \cdot v) W v_i\right) \\
 *     &= \left(\rho h + \frac{B^2}{W^2}\right) W^2 v_i - (B \cdot v) B_i
 * \end{aligned}
 * \f]
 *
 ...
 * \f[
 * \begin{aligned}
 * S^i &= \left(\rho h + \frac{B^2}{W^2}\right) W^2 v^i - (B \cdot v) B^i \\
 * S \cdot B &= \left(\rho h + \frac{B^2}{W^2}\right) W^2 (B \cdot v)
 *              - (B \cdot v) B^2 \\
 *          &= \rho h W^2 (B \cdot v) \\
 * v^i &= \frac{1}{\sqrt{\gamma} (\rho h W^2 + B^2)} \tilde{S}^i
 *       + \frac{S \cdot B}{\rho h W^2 (\rho h W^2 + B^2)}
 * \end{aligned}
 * \f]
 */
