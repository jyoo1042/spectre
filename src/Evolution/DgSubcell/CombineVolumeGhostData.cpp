...
namespace {
template <size_t Dim>
void fill_combined_data(gsl::not_null<DataVector*> extended_data,
                        const DataVector& volume_data,
                        const DataVector& ghost_data, size_t ext_offset,
                        size_t vol_offset, size_t ghost_offset,
  ...
  const bool lower_side = direction_to_extend.side() == Side::Lower;
  const size_t dir = direction_to_extend.dimension();
  ...
  for (size_t i_vol = 0; i_vol < num_vol_pts; ++i_vol) {
    ...
    if (lower_side) {
      multi_idx[dir] += ghost_extents[dir];
    }
...
DataVector combine_volume_ghost_data(
  ...
  ghost_extents[dir] = ghost_zone_size;

  Index<Dim> ext_extents = subcell_extents;
  ext_extents[dir] += ghost_zone_size;
