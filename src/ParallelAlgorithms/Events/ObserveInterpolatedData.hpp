  ...
  static void call_operator_impl(
    ...
    if ((lower_bound_bl <= interp_val) and (upper_bound_bl > interp_val)) {
      ...
      const auto record_tensor_component_impl =
          [&components, &interpolant, &elm_interp_val, &mesh](
            ...
            for (size_t i = 0; i < tensor.size(); ++i) {
              ...
              // const auto wow = logical_coordinates(mesh);
              // CAPTURE_FOR_ERROR(tensor_component);
              // CAPTURE_FOR_ERROR(tensor[i]);
              // CAPTURE_FOR_ERROR(elm_interp_val);
              // CAPTURE_FOR_ERROR(wow);

              if (tag_name == "RestMassDensity" and min(tensor_component) < 0) {
                ERROR("negative rho");
              }
