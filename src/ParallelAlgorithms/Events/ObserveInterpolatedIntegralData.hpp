 ...
 public:
  ...
  static void call_operator_impl(
    ...
    if ((lower_bound_bl < interp_val) and (upper_bound_bl >= interp_val)) {
      ...
      //   const auto jac_interpolated = interpolant.interpolate(jac);

      // Assuming Grid = Inertial right now
      // transform from ElementLogical to Inertial using the jac above
      // basically, there is an issue of jacobian defined over all grid
      // points whereas I only need it for target grid points
      // way around this is take jacobian and interpolate to target points
      // using interpolant above, however, interpolant takes either datavector
      // or double, and since our jacobian structure for simulation
      // is const, just simply take first element.
      ...
      using hideous_tuples = tuples::tagged_tuple_from_typelist<metric_tags>;
      auto metric_quantities =
          call_with_dynamic_type<hideous_tuples, derived_classes>(
              initial_data_ptr, [&qwer](const auto* const data_or_solution) {
                return evolution::Initialization::initial_data(
                    *data_or_solution, qwer, 0.0, metric_tags{});
              });
      ...
      const auto shiftr =
          get<0>(get<gr::Tags::Shift<DataVector, 3, Frame::Inertial>>(box));
      ...
      const auto vr = get<0>(
          get<hydro::Tags::SpatialVelocity<DataVector, 3, Frame::Inertial>>(
              box));
      const auto tvr = lapse * vr - shiftr;
      ...
      const auto record_tensor_component_impl =
          [&interpolant, &local_volume, &magnetic_flux, &surface_integral_value,
           &new_mesh, &det_jacobian, &rho, &vr, &lf, &sqrt_det_metric, &tvr,
           &metric_quantities](const auto& tensor) {
            ...
            const auto new_tvr = interpolant.interpolate(tvr);
            // this one, right now, is the MassFlux first component.
            const double xox =
                definite_integral(new_tensor * new_det_jacobian, new_mesh);
            surface_integral_value += xox;

            // surface area of the surface
            // for now, let's use this for where we interpolate the
            // transport velocity and integrate mass flux computed from that
            const auto shift =
            ...
            const auto detsqrtsm =
                get(get<gr::Tags::SqrtDetSpatialMetric<DataVector>>(
                    metric_quantities));
            DataVector integrand = new_rho * new_lf * detsqrtsm * new_tvr;
            // static_assert(std::is_same_v<decltype(integrand),void>);
            ...
            // magnetic flux
            // where all the metric components are computed and rest is
            // interpolated and reasseembled
            DataVector yolo =
                new_rho * new_lf * detsqrtsm * (lapse * new_vr - shift);
            // static_assert(std::is_same_v<decltype(yolo),void>);
            for (size_t i = 0; i < yolo.size(); i++) {
              if (yolo[i] > 0) {
                yolo[i] = 0;
              }
            }
            const double zoz =
                definite_integral(yolo * new_det_jacobian, new_mesh);
            magnetic_flux += zoz;
          };
